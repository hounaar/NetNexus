...
const as10 = document.querySelector(".as10"),
as10Btn = as10.querySelector(".as10Btn");
...
as10Btn.onclick = ()=>{
    let xhr = new XMLHttpRequest();
    xhr.open("POST", "/src/php/arkham/as10.php", true);
    xhr.onload = ()=>{
      if(xhr.readyState === XMLHttpRequest.DONE){
          if(xhr.status === 200){
              let data = xhr.response;
              if(data === "Success"){
                ...
                const audio = new Audio('/src/audio/10.mp3');
                audio.volume = 0.5; // Adjust volume if needed
                audio.play();
    ...
    let as10_formData = new FormData(as10);
    xhr.send(as10_formData);
}
